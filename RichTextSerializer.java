    ...
    public static String toJson(SpannableStringBuilder ssb) {
        try {
            ...
            for (Object span : allSpans) {
                ...
                if (span instanceof ResizableImageSpan) {
                    ResizableImageSpan ris = (ResizableImageSpan) span;
                    s.put("type", "image");
                    s.put("path", ris.getImagePath());
                    s.put("width", ris.getDisplayWidth());
            ...
            for (int i = 0; i < spans.length(); i++) {
                ...
                switch (type) {
                    ...
                    case "bullet":
                        ...
                        break;
                    case "image":
                        String path = s.optString("path", null);
                        int width = s.optInt("width", 0);
                        if (path != null && width > 0 && new java.io.File(path).exists()) {
                            ResizableImageSpan imgSpan = ResizableImageSpan.create(path, width);
                            if (imgSpan != null) {
